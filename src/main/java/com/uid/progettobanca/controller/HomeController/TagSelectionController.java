...
public class TagSelectionController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // get the transaction from the Stack in TransactionManager
        // (transaction was previously pushed in the stack in TransactionDetailsController)
    ...
    private void setGenericTag(String tag) {
        ...
        transactionService.setOnSucceeded(e -> {
            if(e.getSource().getValue() instanceof Boolean result){
                if(result){
                    // reloadDynamicPageInHashMap is used to update information where needed
                    //(e.g. transactions in the home page, charts, etc.)
    ...
    // listener for the click on the tag boxes
    @FXML
    void setAltro(MouseEvent event) { setGenericTag("Altro"); }
    @FXML
